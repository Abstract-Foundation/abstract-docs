...
The native currency on Abstract is Ether (ETH). Other currencies including USDC.e and USDT are also available.
    ...
    4. Double check everything looks correct. **Transactions cannot be reversed, and funds sent to the wrong places are impossible to recover**
...
<Warning>
  ...
  AGW only exists in the Abstract.
</Warning>
...
<AccordionGroup>
  <Accordion title="How do I edit my profile?">
    Navigate to "Profile Page" in the left-hand menu to:
...
<Accordion title="Why do I need to connect my Discord account to get rewards?">
  Social connections enhance ecosystem security by: - Providing an additional layer
...
<Accordion title="What is slippage?">
  Slippage is the difference between the expected trade price and the actual execution
...
<Accordion title="What happens if I lose my account credentials?">
  ...
  passkeys - Enabling 2FA Both are found in the settings
</Accordion>
