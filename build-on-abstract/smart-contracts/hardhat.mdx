...
<Tip>
  **Starting a new project?** Consider using the [ZKsync CLI
  `create`](/build-on-abstract/zksync-cli) command to initialize a Hardhat new
  project with all the necessary dependencies & configurations along with smart
...
Write a new smart contract in the `contracts/HelloAbstract.sol` file, for example:
...
(installed in the above step) to compile smart contracts for Abstract:
