...
<Tip>
  **Starting a new project?** Consider using the [ZKsync CLI
  `create`](/build-on-abstract/zksync-cli) command to initialize a new Hardhat
  project with all the necessary dependencies & configurations setup, along with
  some example smart contracts and scripts to get started with.
</Tip>
...
## 2. Install the required Hardhat extension
...
To compile smart contracts for Abstract, install the
[hardhat-zksync](https://docs.zksync.io/build/tooling/hardhat/hardhat-zksync) extension.

```bash
npm install -D @matterlabs/hardhat-zksync
```

To interact with [system contracts](/how-abstract-works/system-contracts/overview) install the
[zksync-contracts](https://docs.zksync.io/build/tooling/hardhat/hardhat-zksync#zksync-contracts) package:

```bash
npm install -D @matterlabs/zksync-contracts
```

To write [Hardhat scripts](https://hardhat.org/hardhat-runner/docs/advanced/scripts), it&rsquo;s recommended to install
the [zksync-ethers](/build-on-abstract/applications/ethers) package:
...
<Tip>
  For local development, the [zksync-cli](/build-on-abstract/zksync-cli) can run
  local Abstract nodes which can also be configured as a network in the Hardhat
  configuration file.
</Tip>
...
```
  ...
  interact with it from a [block explorer](/ecosystem/block-explorers).
