...
<Note>Or you can use `--private-key <YOU PRIVATE KEY>` instead of **`--account myKeystore`** as well.</Note>
...
```
  ...
  <Step title="Verify an existing smart contract" icon="check">

In the above step, we verified the smart contract immediately after deployment.

To verify an existing smart contract on the [block explorer](/tooling/block-explorers), run the following command:
