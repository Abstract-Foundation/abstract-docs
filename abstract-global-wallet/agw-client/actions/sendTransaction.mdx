...
It extends the [sendTransaction](https://viem.sh/zksync/actions/sendTransaction) function from Viem to include support for
...
```tsx
import { sendTransaction } from "@abstract-foundation/agw-client";
import { agwClient } from "./config";
import { abstractTestnet } from "viem/chains";
...
Returns a `Promise<Hash>` containing the transaction hash of the submitted transaction.
