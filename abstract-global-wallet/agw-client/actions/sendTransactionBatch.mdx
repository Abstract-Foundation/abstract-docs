...
includes a `sendTransactionBatch` method that can be used to sign and submit multiple transactions in a single batch to the chain using the connected Abstract Global Wallet.
...
Returns a `Promise<Hash>` containing the transaction hash of the submitted transaction batch.
