...
The `toSessionClient` creates a new instance of the [AbstractClient](/abstract-global-wallet/agw-client/createAbstractClient) instantiated with the session key as the signer, allowing you to submit transactions and perform action (e.g. [writeContract](/abstract-global-wallet/agw-client/actions/writeContract)) from a session key.
