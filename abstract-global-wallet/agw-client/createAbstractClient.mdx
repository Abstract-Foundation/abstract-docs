...
```tsx
import { createAbstractClient } from "@abstract-wallet/agw-client";
import type { AbstractClient } from "@abstract-wallet/agw-client"; // Optional
```
...
const account: Account = privateKeyToAccount(randomPrivateKey); // Use that to create a signer
...
<ResponseField name="address" type="ChainEIP712" required>
