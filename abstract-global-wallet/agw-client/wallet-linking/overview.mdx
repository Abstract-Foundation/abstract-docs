...
<AccordionGroup>
  <Accordion title="exclusiveWalletByRights">
    Given an AGW address as input, returns either:
    - ✅ If the AGW has an EOA linked: the EOA address.
    - ❌ If the AGW does not have an EOA linked: the AGW address.

    Use this to check if an AGW has an EOA linked.
  ...
  <Accordion title="delegatedWalletsByRights">
    Given an EOA address as input, returns a list of L1 wallets that have linked to the AGW.
    Use this to check what AGWs an EOA has linked (can be multiple).

    ```solidity
    function delegatedWalletsByRights(
      address wallet,   // The wallet to check delegations for
      bytes24 rights   // The rights identifier to check
    ) returns (address[])
    ```
