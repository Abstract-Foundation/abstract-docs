...
1. The price for transaction execution fluctuates as it depends on the price of Ethereum gas price.
...
However, if the address you call is a smart contract (which all accounts on Abstract are),
...
field that can be set to control the amount of gas that is charged for eaach byte of data sent to L1
