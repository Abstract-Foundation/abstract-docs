...
[IAccount](#iaccount-interface) interface. As outlined in the [transaction flow](./transaction-flow.mdx),
...
Typically, you would perform some kind of [signature validation](./signature-validation.mdx) in this step
to restrict who can use the account. This function must:

1. Increment the nonce for the account. See [handling nonces](./handling-nonces.mdx) for more information.
2. Return `ACCOUNT_VALIDATION_SUCCESS_MAGIC` if the transaction is valid and should be executed.
