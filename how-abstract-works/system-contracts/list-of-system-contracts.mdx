...
whenever the VM calls accesses an `address`.
...
- Compresses the published bytecodes.
- Compresses the state diffs (and validate state diff compression).
...
callable so that the users could transfer ETH to it.

For these contracts, we insert the EmptyContract code upon genesis.
...
standard Solidity `emit` keyword can be used to emit events.
...
it is used indirectly via the compiler simulations for the immutable variables in Solidity.
...
(see [AccountCodeStorage](#accountcodestorage)), the system must ensure that
...
Learn more about what data is sent in the [transaction lifecycle](/how-abstract-works/architecture/transaction-lifecycle) section.
...
While you don&rsquo;t need to interact with this contract directly, it does impact
the support for `.send` and `.transfer` functions in Solidity.
Learn more on the
[recommended Solidity changes](/how-abstract-works/evm-differences/recommended-solidity-changes) page.
...
This contract stores the nonce for each account on the L2. More specifically, stores both
...
See more details in the [handling nonces](/how-abstract-works/native-account-abstraction/handling-nonces) section.
