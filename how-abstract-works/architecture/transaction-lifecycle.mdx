...
<Steps>
  <Step title="Abstract (Processed)">
    ...
    Multiple batches are committed to the Ethereum blockchain in a 
    single transaction
    ...
    **<Tooltip tip="State diffs offer a more cost-effective approach to transactions than full 
    ...
    calling the `commitBatches` function on the [L1 smart contract](/how-abstract-works/architecture/l1-rollup-contracts) and ensuring their [data availability](https://ethereum.org/en/developers/docs/data-availability/).
  ...
  <Step title="Ethereum (validating)">
    ...
    Ethereum smart contract for verification by calling the
    `proveBatches` function on the [L1 smart contract](/how-abstract-works/architecture/l1-rollup-contracts). 

    This process involves both the [prover](/how-abstract-works/architecture/components/prover-and-verifier) responsible for 
    generating the proof off-chain in the form of a [ZK-SNARK](https://ethereum.org/en/developers/docs/scaling/zk-rollups/#validity-proofs)
    & submitting it to the [L1 smart contract](/how-abstract-works/architecture/l1-rollup-contracts) and the [verifier](/how-abstract-works/architecture/components/prover-and-verifier)
    to confirm the validity of the proof on-chain.
  ...
  <Step title="Ethereum (executing)">
    ...
    by calling the `executeBatches` function on the L1 smart contract. [Learn more about state commitments](https://ethereum.org/en/developers/docs/scaling/zk-rollups/#state-commitments).
