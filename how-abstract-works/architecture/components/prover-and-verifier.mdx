...
description: "Learn more about the prover component of Abstract."
---

The batches of transactions submitted to Ethereum by the [sequencer](/how-abstract-works/architecture/sequencer)
are not necessarily valid (i.e. they have not been proven to be correct) until a ZK proof is generated
and verified by the [L1 smart contract](/how-abstract-works/architecture/l1-rollup-contracts).
...
2. **[Proof verification](#proof-verification)**: The proof is submitted to the [L1 smart contract](/how-abstract-works/architecture/l1-rollup-contracts) and verified by the **on-chain** verifier.
...
<Steps>
  <Step title="Witness Generation">
    A **witness** is the cryptography term representing the knowledge 
    that the prover wishes to demonstrate is true. 
    
    In the context of Abstract, the witness is the data that the prover claims  
    a transaction is valid. It does this without disclosing any transaction details.
  ...
  <Step title="Circuit Execution">
    ...
    covering every opcode, storage interaction, and the integration of precompiled contracts. 
  ...
  <Step title="Proof Compression">
    ...
    For this reason, a final compression step is to generate a succint validity proof
...
[L1 smart contract](/how-abstract-works/architecture/l1-rollup-contracts) as outlined in the 
